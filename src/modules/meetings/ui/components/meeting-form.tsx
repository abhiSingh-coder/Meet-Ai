...
interface MeetingFormProps {
    ...
    onCancel?: () => void,
    initialVlaues?: MeetingGetOne
}

export const MeetingForm = ({ onCancel, onSuccess, initialVlaues }: MeetingFormProps) => {
    const trpc = useTRPC()
    ...
    const updateMeeting = useMutation(trpc.meetings.update.mutationOptions({
        onSuccess: async () => {
            ...
            if (initialVlaues?.id) {
                await queryClient.invalidateQueries(
                    trpc.meetings.getOne.queryOptions({ id: initialVlaues.id })
                )
            }
            onSuccess?.()
        },
    ...
    const onSubmit = (values: z.infer<typeof meetingsInsertSchema>) => {
        if (isEdit)
            updateMeeting.mutate({ ...values, id: initialVlaues.id })
        else
